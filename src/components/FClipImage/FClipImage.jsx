...
export default defineComponent((props, { expose }) => {
    ...
    const { isImage, isPreview, isFile, image, bgClip, clip, previewImage, clipScale, imageWidth, imageHeight, originalImageWidth, originalImageHeight, scaleX, scaleY, curCanvas, clipPath, } = toRefs(state);
    let x = 0;
    let y = 0;
    let clipX = 0;
    let clipY = 0;
    ...
    function changeImgae(e) {
        ...
        reader.onload = async (e) => {
            ...
            bgClip.value.src = e.target.result;
    ...
    function setClipSize(width, height) {
        if (!clip.value) {
            return;
        }
    ...
    function initIsPreview() {
        ...
        isPreview.value = props.isPreview;
    }
    initIsPreview();
    async function setClipPath() {
        ...
        const rt = `${clip.value.offsetLeft + clip.value.offsetWidth}px ${clip.value.offsetTop}px`;
        const lb = `${clip.value.offsetLeft}px ${clip.value.offsetTop + clip.value.offsetHeight}px`;
        const rb = `${clip.value.offsetLeft + clip.value.offsetWidth}px ${clip.value.offsetTop + clip.value.offsetHeight}px`;
    ...
    function executeClipImage() {
        ...
        const resp = clipImage(clip.value.offsetLeft * scaleX.value, clip.value.offsetTop * scaleY.value, clip.value.offsetWidth * scaleX.value, clip.value.offsetHeight * scaleY.value, clip.value.offsetWidth, clip.value.offsetHeight);
        return resp;
    }
    async function getClipDataUrl(fn) {
        if (!isPreview.value) {
            const resp = executeClipImage();
            if (!resp) {
                return console.warn("resp is null");
    ...
    async function getClipFileUrl(fn) {
        if (!isPreview.value) {
            executeClipImage();
        }
        const file = await getClipFile();
    ...
    function clipImage(x, y, width, height, canvasWidth, canvasHeight) {
        const canvas = document.createElement("canvas");
        canvas.width = canvasWidth;
        canvas.height = canvasHeight;
        const ctx = canvas.getContext("2d");
        ctx?.drawImage(image.value, x, y, width, height, 0, 0, canvasWidth, canvasHeight);
    ...
    function zoomWindowMouseMove(e, key) {
    ...
    function restrictClipSize(w, h, x, y) {
        let width = w;
        let height = h;
        let left = x;
        let top = y;
    ...
    function clipRefMouseDown(e) {
        x = e.clientX;
        y = e.clientY;
        clipX = clip.value?.offsetLeft;
        clipY = clip.value?.offsetTop;
        offset.x = x - clipX;
        offset.y = y - clipY;
    ...
    function zoomHandler(childNodes) {
        const zoomMap = {
            lt: childNodes[0],
            rt: childNodes[1],
            lb: childNodes[2],
    ...
    return () => (<ClipBox vars={{
        ...
        {isFile.value ? (<div class="clip-container">
            <div class="clip-box">
          ...
          </div>) : (<div class="add-box" onClick={selectImage}>
            <div class="add">+</div>
            <input accept="image/*" ref={inputRef} type="file" onChange={changeImgae}/>
          </div>)}
      </ClipBox>);
}, {
    props: [
...
//# sourceMappingURL=FClipImage.jsx.map