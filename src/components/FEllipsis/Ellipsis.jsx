...
import { useElementBounding, useWindowSize, useWindowScroll, useMutationObserver, useResizeObserver, } from "@vueuse/core";
import { ellipsisApiInjectionKey } from "./context";
import { defineComponent, onMounted, onUnmounted, ref, reactive, toValue, watch, computed, provide } from "vue";
export default defineComponent((props, { slots }) => {
    const triggerInnerRef = ref(null);
    const ellipsisClass = ref("");
    const triggerRef = ref(null); // 触发组件的ref实例
    ...
    const { stop: stopResizeTriggerRef } = useResizeObserver(triggerRef, initEllipsis);
    const { stop: stopMutationTriggerInnerRef } = useMutationObserver(triggerInnerRef, initEllipsis, {
        characterData: true,
        subtree: true,
    });
    function isResolveSlotText() {
        ...
        if (Array.isArray(s)) {
            if (s && typeof s.at(0)?.type !== "symbol") {
                stop();
                return console.error("Incoming slot contents cannot be wrapped by nodes");
            }
            if (s && s.length > 1) {
                stop();
    ...
    function stop() {
        stopResizeTriggerRef();
        stopMutationTriggerInnerRef();
    }
    onMounted(() => {
        isResolveSlotText();
        initEllipsis();
    ...
    onUnmounted(stop);
    function specialPosition(triggerClientRef, state, pn) {
        const ys = windowY.value > 0
        ...
        if (toValue(state.tootipWidth) > position.x + state.tootipWidth / 2) {
            ...
            return `translate(${toValue(position.x)}px, ${toValue(ys + (pn === "bottom" ? toValue(triggerClientRef.height) : 0))}px) ${pn === "bottom" ? "" : "translateY(-100%)"}`;
        }
        if (windowWidth.value - triggerClientRef.right.value <
            state.tootipWidth / 2) {
            state.setTriangle("right");
            state.setOriginPosition(`${pn === "bottom" ? "top" : "bottom"} right`);
            return `translate(${toValue(position.x) -
                toValue(state.tootipWidth - triggerClientRef.width.value)}px, ${ys + (pn === "bottom" ? toValue(triggerClientRef.height) : 0)}px) ${pn === "bottom" ? "" : "translateY(-100%)"}`;
        }
        if (pn === "top") {
            return `translate(${toValue(position.x) + toValue(triggerClientRef.width) / 2}px, ${ys}px) translateY(-100%) translateX(-50%)`;
        }
        else {
            return `translate(${toValue(position.x) + toValue(triggerClientRef.width) / 2}px, ${ys + toValue(triggerClientRef.height)}px) translateX(-50%)`;
        }
    }
    function getPosition(triggerClientRef, state) {
    ...
    const omit = computed(() => ellipsisClass.value === "ellipsis" ? ellipsis : "");
    return () => (<div ref={triggerRef} onMouseenter={mouseEnter} onMouseleave={mouseLeave} style={{
            whiteSpace: "nowrap",
            ...omit.value,
        }}>
...
//# sourceMappingURL=Ellipsis.jsx.map