 ...
 */
export type RequiredObj<T, V extends keyof T> = Required<Pick<Partial<T>, V>> &
  Omit<Partial<T>, V>;
export type Render = number | string | (() => VNodeChild);
export type ReturnTypeCallBack<T = unknown> = (e: MouseEvent) => T;
export type NonRefKeys = "removeModal" | "anotherNonRefField"; // 定义不需要转换为 Ref 的属性名联合类型
export interface IProps {
  ...
  action: () => VNodeChild; //操作区域的内容，需要是 render 函数
...
export type DialogInstance = (
  data: RequiredObj<IProps, "content">
) => ModalFuncReturn;

export type DialogProps = RequiredObj<IProps, "content">;
