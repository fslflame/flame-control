...
import { Transition, createApp, withModifiers, defineComponent, ref, reactive, computed, onMounted, onUnmounted, toRefs, } from "vue";
 ...
 * 弹出框组件
 */
const MyShowModel = defineComponent((props) => {
    const { content, clickPositon, removeModal, loading, negativeText, positiveText, onNegativeClick, onPositiveClick, title, type, action, maskClosable, transformOrigin, closeOnEsc, } = props;
     ...
     * 渲染元素？
     */
    const renderJsx = (el) => (isShowModal.value ? el : null);
    onMounted(() => {
    ...
    function handleLoadingLeave(e) {
        const el = e;
        el.style.maxWidth = "0";
        el.style.marginRight = "0";
    }
    function handleLoadingBeforeLeave(e) {
        const el = e;
    ...
    return () => (<ModalContainer>
        <Transition appear name="fade" mode="out-in">
          {renderJsx(<ModalMask onClick={withModifiers(closeModal(() => {
                maskClosable.value ? hide() : null;
            }), ["self"])}></ModalMask>)}
        </Transition>
        <ModalWrapper onScroll={modalScrollHandler}>
          <Transition onAfterLeave={removeModal} onEnter={handleEnter} appear name="zoom-out" mode="out-in">
            {renderJsx(<ModalDiv style={{
                transformOrigin: isTransformOrigin.value,
            }}>
                <>
                  <div class="icon">
                    {type.value ? (<SvgIcon name={iconMap[type.value].icon} size={iconMap[type.value].size}></SvgIcon>) : null}
                    <div>{title.value}</div>
                  </div>
                  <div class="content">
                ...
                <div class="action">
                  {action ? (action.value()) : (<>
                    ...
                    type: "cancel",
                }} onClick={handlerPropsClick(onNegativeClick && onNegativeClick.value)}>
                        {negativeText.value}
                      </ActionButton>
                      <ActionButton vars={{
                    opacity: `${loading.value ? 0.5 : 1}`,
                    cursor: `${loading.value ? "not-allowed" : "pointer"}`,
                }} variants={{ theme: isTheme() }} disabled={loading.value} onClick={handlerPropsClick(onPositiveClick && onPositiveClick.value)}>
                        <Transition onEnter={handleLoadingEnter} onLeave={handleLoadingLeave} onBeforeLeave={handleLoadingBeforeLeave} name="loading" appear>
...
const dialog = (data) => modalHandler(data);
dialog.info = (data) => {
    data.type = "info";
...
export function createModalApp(data) {
    ...
    const handleClickOnce = (event) => {
        if (_isClickPosition(stateAsRefs.clickPositon?.value?.x, stateAsRefs.clickPositon?.value?.y)) {
            return;
        }
